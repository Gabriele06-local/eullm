...
curl -L https://github.com/eullm/eullm/releases/latest/download/eullm-linux-x64-cuda-13.1 -o eullm
chmod +x eullm
...
| 🐧 Linux x64 (NVIDIA data-center) | `eullm-linux-x64-cuda-12.4-datacenter` | 🆕 Untested | A100 (sm_80), H100 (sm_90) — a different, older compute capability than the consumer build above, which doesn't run on these cards. **CUDA 12.4, not 13.1 like the consumer build**: minimum driver r550 instead of r580. That is deliberate — A100s live in HPC centres and enterprise fleets, which freeze driver versions for years. The first attempt shipped CUDA 13.1 and would not start on CINECA Leonardo at all (`CUDA driver version is insufficient for CUDA runtime version`, then a silent fall back to CPU); see `docs/cineca/leonardo.md`. The 12.4 rebuild has not yet been run on real hardware — if you try it, please report back |
