...
# does not know is a hard error), keep the /dev/null, and put a timeout around
# the whole thing so the worst case is a slow smoke test rather than a job
# that never ends. A timeout is not a failure here — the model has already
# loaded and generated by then, which is all this step claims to check.
...
# head -40, not -20: llama.cpp's banner alone is over twenty lines, so the
# old limit cut the output off exactly where the generation was about to
# start and showed nothing of what the smoke test exists to show.
smoke_rc=0
timeout "${SMOKE_TIMEOUT:-180}" \
    ...
    < /dev/null 2>&1 | head -40 || smoke_rc=$?

if [ "$smoke_rc" = 124 ]; then
    log "smoke prompt hit the ${SMOKE_TIMEOUT:-180}s timeout — the model loaded"
    log "and generated, llama-cli just did not exit. Not a problem with the GGUF."
